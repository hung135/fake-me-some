        ...
        if self._sqlalchemy_con is None or self._sqlalchemy_meta.get(schema, None) is None:
            ...
            if db_type.upper() == "POSTGRES":
                ...
                if self._sqlalchemy_con is None:
                    self._sqlalchemy_con = sqlalchemy.create_engine(self.url, client_encoding="utf-8",fast_executemany=True,
...
group by relname;""".format(table_name)
    ...
    def print_table_info(self, table_name, dbschema):
        ...
        Base.prepare(engine, reflect=True, schema=dbschema)
  ...
  SELECT *, total_bytes-index_bytes-COALESCE(toast_bytes,0) AS table_bytes FROM (
    ...
    def set_table_owner(self, table_name_fqn, role):
        if self._dbtype == 'POSTGRES':
            ...
            raise Exception('Not Supported Databse Type;{}'.format(self._dbtype))
    ...
    def import_file_client_side(self, full_file_path, table_name_fqn, file_delimiter=',', header=False, encoding='utf8'):
        ...
        return i[1]
    #place holder to push data to redshift..
    #this uses aws s3 copy command to quick push data
    def upload_to_redshift(self,file_name):
        import odo
        table = odo(file_name, 'redshift://user:pass@host:port/db::users')
    #fake some data up
    def faker_data(self,table_name,num_rows=100):
        #https://github.com/joke2k/faker/tree/master/faker/providers
        from faker import Faker
        fake = Faker()
        from sqlalchemy import MetaData, Table, Types
        from sqlalchemy.engine.reflection import Inspector
        def map_faker(column_name,c_data_type):
            for f in Faker.providers.keys():
                if f==column_name:
                    return Faker.providers[f]
            if isinstance(Types.Boolean,c_data_type):
                return Faker.boolean
            if isinstance(Types.BigInt,c_data_type):
                return Faker.Integer
            
            return Faker.providers.lorem
        engine = self.connect_sqlalchemy()
        meta = MetaData() 
        insp = Inspector.from_engine(engine)
        
        curr_table = Table(table_name, meta, autoload=True, autoload_with=engine)
        insp.reflecttable(curr_table, None)
        columns=[c.name for c in curr_table.columns]
        columns_datatype=[c.data_type for c in curr_table.columns]
        faker_func=[map_faker(c.name,c.data_type) for c in curr_table.columns]
        ...
        for _ in range(num_rows):
            columns_dict={}
            for col,faker_provider_func in columns, faker_func:
                columns_dict[col]=faker_provider_func() 
        
            curr_table.insert().values(columns_dict)
